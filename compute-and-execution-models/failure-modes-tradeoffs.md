...
- Understand the underlying causes and characteristics of systemic failure patterns.
...
When we run our code on a virtual machine, we are responsible for almost everything in it including the operating system, network configurations, the runtime environment, and the application code. Since we have so much control, there are many ways the system can degrade. We won't look at every single way that IaaS resources can fail, but we can broadly group them into 3 categories of failure.
...
- The Problem: The startup time includes provisioning a container, downloading your code package, and initializing the runtime environment, which creates a sudden, one-time spike in latency. While your code might normally run in 100ms, for example, a cold start can push that execution time to a few seconds or more.
...
##### !options
a| This is a provider failure. The cloud service provider’s virtualization layer is malfunctioning.
b| This is a customer failure. The responsibility lies in the system's permissions configuration.
c| This is an OS-level failure. You must SSH into the function environment to clear the credential cache.
d| This is a blind spot failure. Since you lack visibility into the network stack, you cannot determine the cause.
##### !end-options
...
##### !options
a| IaaS because it ensures cost predictability and avoids cloud bursting.
b| PaaS because it maintain high visibility into the OS for the weekly 15-minute window.
c| FaaS because it eliminates idle resources by scaling to zero during the rest of the week.
d| FaaS because cloud bursting is always cheaper than a VM regardless of traffic volume.
##### !end-options
