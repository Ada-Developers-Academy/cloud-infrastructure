...
| Declarative Automation | Describing the final state we want and letting the system figure out how to achieve it. For example: "There should be 3 servers running," rather than writing step-by-step instructions to create them. | Declarative configuration, desired-state configuration | "We wrote a declarative automation template that specifies our application needs four running instances, and the platform handles adding or removing servers to match that target." |
...
**Infrastructure-as-Code** (IaC) is the practice of defining and managing infrastructure, like servers, databases, network settings, permissions, and more, through code and configuration files rather than by clicking through a dashboard or running commands by hand. IAC can be considered part of **Systems Automation**: a collection of processes that automate operational tasks like deployments, backups, scaling, and updates so humans don’t have to perform repetitive manual work. Instead of logging into a console and manually provisioning a database instance, an engineer writes a file that describes what the database infrastructure should look like. That file is then read by a tool that creates or updates the real infrastructure to match.
...
The core idea of Infrastructure-as-Code is declarative configuration: we describe what we want the infrastructure to look like, and the IaC tool determines what needs to be created, updated, or left alone to reach that state. This is different from writing imperative step-by-step scripts. The other options either describe manual processes, mischaracterize how IaC tools work, or confuse IaC with fully managed services.
