...
Within a storage transfer service, there are typically a few offerings aimed at moving different kinds of storage. A single storage transfer service is likely to have options specific to block storage, file storage, and even in-person transfer (where large storage volumes are delivered to the customer to fill, then physically sent back to the cloud vendor for upload) for massive amounts of data or sensitive data that would be costly or otherwise not allowed to be sent across networks. 
