...
The **Principle of Least Privilege (PoLP)** states that any principal should only be granted the minimum permissions necessary to perform its intended function, and nothing more. It is one of the most foundational principles in security and applies to every type of principal in an IAM system: human users, service accounts, groups, roles, automated processes, and workloads alike. Applying the Principle of Least Privilege means scoping permissions as narrowly as possible along three dimensions:
