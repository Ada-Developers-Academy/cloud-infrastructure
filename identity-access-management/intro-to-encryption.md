...
**Symmetric encryption** is used when the primary requirement is encrypting large volumes of data efficiently, and the parties involved can securely share a key in advance. Encrypting a database, a file system, or a backup archive are all scenarios where symmetric encryption is the appropriate choice. **Asymmetric encryption** is used when the parties involved have not previously shared a secret and need to establish a secure channel, or when authentication and data integrity guarantees are required. Verifying the identity of a server during a TLS handshake, authenticating access to a remote system with SSH keys, and signing software releases with a digital signature are all scenarios where asymmetric encryption is the appropriate choice.
...
**Generation** is the process of creating a new encryption key. Keys must be generated using a cryptographically secure process that produces values that are sufficiently random and unpredictable. A key that is generated using a weak or predictable process is vulnerable to being guessed or reconstructed by an attacker, regardless of how strong the encryption algorithm itself is.
